...
//==> Tabla principal
  $resultUpd = mysqli_query($Con,"SELECT
          ...
          WHERE fecha_cancelado between cast('$fesInicio' as datetime) AND cast('$fesFinal' as datetime)");
//==> Tabla Espacio
  $resultEsp = mysqli_query($Con,"SELECT          spf.nombre AS espacio_fis
                                    FROM tbl_evento_detalle    AS evt
                                    JOIN tbl_espacios_fisicos  AS spf ON spf.idRow = evt.espacios_fisicos_id
                                    JOIN tbl_eventos           AS evn ON evn.id    = evt.idRow
                                    WHERE fecha_cancelado between cast('$fesInicio' as datetime) AND cast('$fesFinal' as datetime)");
//==> Tabla Material
...
//==> Tabla Peticiones
  $resultPet = mysqli_query($Con,"SELECT pte.descripcion AS peticion,
                                  ...
                                  JOIN tbl_usuarios      AS usu ON usu.idRow = evn.usuarios_id
                                    WHERE fecha_cancelado between cast('$fesInicio' as datetime) AND cast('$fesFinal' as datetime)");
//==> Tabla de todas las Cancelaciones
$resultCan = mysqli_query($Con,"SELECT  evn.title      as evento_cancelado,
          ...
          <div align="center">
          <hr>
          <h3>Reporte de sistema por eventos cancelados</h3>
          <hr>
          </div>
          <div align="center">
              <table width="95%">
          ...
          <div align="center">
              <table width="95%">
                <tr>
                  <td bgcolor="#00785F"><strong>Espacios reservados </strong></td>
                </tr>';
...
$dompdf->render();
$dompdf->stream("ReporteGeneralCancelados.pdf");
  ...
  <script type='text/javascript'>
   alert('Error! fechas vacias'); location.href = '../rep_sistem.php';
  </script>";
}
